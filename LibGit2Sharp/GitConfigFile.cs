...
{
    /// <summary>
    /// Use this class to retrieve a configuration variable stored in a loned file.
    /// </summary>
    /// <remarks>
    /// version réduite
    /// This is a scaled-down version of the <see cref="Configuration"/> optimized for single file access.<br/>
    /// If you need to retrieve any repository variable, use the original <see cref="Configuration"/> class.
    /// </remarks>
    public class GitConfigFile : IDisposable, IEnumerable<ConfigurationEntry<string>>
    {
        private readonly FilePath   configPath;
        private ConfigurationHandle configHandle;

        internal GitConfigFile(string repositoryConfigurationFileLocation)
        {
            configPath   = NormalizeConfigPath(repositoryConfigurationFileLocation);
            configHandle = Proxy.git_config_new();
            Proxy.git_config_add_file_ondisk(configHandle, configPath, ConfigurationLevel.Local, null);
        }
        ...
        public static GitConfigFile BuildFrom(string repositoryConfigurationFileLocation)
        {
            return new GitConfigFile(repositoryConfigurationFileLocation);
        }
